...
dum_slack = 1;      %0 = shorting ban; 1 = no shorting ban;
...
P_guess = (1/K)*EV_sum  - (1/K)*Omega_bar*N_bar;
    ...
    if min_slack >= 0 || dum_slack==1
        ...
        Constrained_LIN
            ...
            if dum_sol==1
                ...
                Demands_opt = V_bar*( EP_adj(:,1:K) - P_guess );
                Demands_opt(:,set_out) = V11*( EP_adj(:,set_out) - P_guess );
                Demands_opt(:,B12_star) = Demands_opt(:,B12_star);
...
figure(1)
subplot(1,2,1), plot(1:length(Demands_1), Demands_1), hold on, plot(1:length(Demands_2), Demands_2), 
hold on, ylabel('Asset demands'), xlabel('Investor'), title('Unrestricted short-selling: K=100')
