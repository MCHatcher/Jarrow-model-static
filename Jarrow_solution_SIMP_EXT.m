...
vec2 = linspace(0.5,1.5,K);

rng(3)
vec = vec(randperm(K));
vec2 = vec2(randperm(K));
...
EV_sum = [ vec2*EP_adj(1,:)'; vec2*EP_adj(2,:)' ];
P_guess = (1/sum(vec2))*EV_sum  - (1/sum(vec2))*Omega_bar*N_bar;

Demands_guess = V_bar*( EP - P_guess ); 
Demands_guess(1,:) = vec2.*Demands_guess(1,:); Demands_guess(2,:) = vec2.*Demands_guess(2,:);
min_slack = min(min(Demands_guess));
    ...
    if min_slack >= 0 || dum_slack==1
        ...
        Demands(1,:) = vec2.*Demands(1,:); Demands(2,:) = vec2.*Demands(2,:);
        ...
        Constrained_SIMP_EXT
            ...
            if dum_sol==1
                ...
                Dem11 = vec2(1,B11).*Dem11;
                Dem22 = V22*(EP_adj(:,B22)-P_guess);
                Dem22 = vec2(1,B22).*Dem22;
                
                Demands = sum(Demands_opt,2) + sum(Dem11,2) +  sum(Dem22,2);
